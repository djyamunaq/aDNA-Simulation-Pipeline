...
def main():
    ...
    subprocess.run(['cp', args.refDNA, os.path.join(os.path.dirname(__file__), './gargammel/data/endo/')])
    # Run gargammel simulation
    subprocess.run([os.path.join(os.path.dirname(__file__),'./gargammel/gargammel.pl'), os.path.join(os.path.dirname(__file__), './gargammel/data/')])
    # Remove output dir if it exists
    subprocess.run(['rm', '-rf', args.output])
    # Create output
    subprocess.run(['mkdir', args.output])
    # Move files to output dir
    subprocess.run(['cp', os.path.join(os.path.dirname(__file__), './gargammel/data/simadna_s1.fq.gz'), os.path.join(args.output, 'simadna_s1.fq.gz')])
    subprocess.run(['cp', os.path.join(os.path.dirname(__file__), './gargammel/data/simadna_s2.fq.gz'), os.path.join(args.output, 'simadna_s2.fq.gz')])
